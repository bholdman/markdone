...
There's also a menu bar icon with the same quick actions plus "Show MarkDone". The app stays resident in the menu bar after you close the window, so the icon and global hotkeys keep working.

## Features

### Capture
- Menu bar icon with quick actions + system-wide hotkeys (Carbon Hot Key API — no Accessibility permission needed)
...
On first launch MarkDone opens an empty document and adds an icon to the menu bar. Closing the window leaves the app running in the menu bar; use the icon's menu or a hotkey to bring it back. Quit from the icon's menu or with ⌘Q.
