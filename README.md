...
There's also a menu bar icon: click it for the same quick actions, double-click it to show MarkDone. The app stays resident in the menu bar after you close the window, so the icon and global hotkeys keep working.

## Features

### Capture
- Menu bar icon (click for actions, double-click to show) + system-wide hotkeys (Carbon Hot Key API — no Accessibility permission needed)
...
On first launch MarkDone opens an empty document and adds an icon to the menu bar. Closing the window leaves the app running in the menu bar; double-click the icon or press a hotkey to bring it back. Quit from the icon's menu or with ⌘Q.
